...
using YumeScript.Translator.Script;

namespace YumeScript.Parser;

internal static class ScriptTranslator
{
    internal static void ParseScript(this RuntimeInstance runtime, Script.Script script)
    {
        ...
        var scriptTree = new ScriptBuilder();
        ...
        for (var i = 0; i < script.SourceCode.Count; i++)
        {
            try
            {
                ...
                while (parserStack.Count > 0 && tabsCount <= parserStack.Peek().Item1)
                {
                    ...
                    scriptTree.SaveCurrentState();
                    
                    // Pass tokens only if frame has the same indention level
                    var indentionParserResult = parserStack.Peek().Item2.FinalizeIndentionSection(scriptTree.Length, tabsCount == parserStack.Peek().Item1 ? tokens : Array.Empty<string>());
                    ...
                    if (indentionParserResult.KeepParser && !tokens[^1].EndsWith(":"))
                    {
                        throw new TranslationException(); //ToDo
                    }
                    
                    //scriptTree.Append(parserResult.Instructions, parserStack.Peek().Item2);

                    if (!indentionParserResult.KeepParser || tabsCount != parserStack.Peek().Item1)
                    {
                        parserStack.Pop();
                    }
                    else
                ...
                if (parserStack.Count > 0)
                {
                    ...
                    instructionsToAppend = activeParser.InterceptLineTokens(scriptTree.Length, tokens, tabsCount - activeParserIndention).Instructions;
                ...
                if (instructionsToAppend == null)
                {
                    foreach (var (priority, parserType) in runtime.InstructionParsers)
                    {
                        var parser = (IInstructionParser)ReflectionHelper
                            .CreateInjectedInstance(parserType, (typeof(IScriptBuilder), scriptTree));
                        parserResult = parser.ParseLineTokens(scriptTree.Length, tokens);
                ...
                {
                    var activeParser = parserStack.Count > 0 ? parserStack.Peek().Item2 : null;
                    // Add current parser to stack only if it was created
                    if (currentParser != activeParser)
                    {
                ...
                if (instructionsToAppend != null)
                {
                    scriptTree.Append(instructionsToAppend, currentParser);
                }
            }
        ...
        while (parserStack.TryPop(out var parserFrame))
        {
            ...
            // scriptTree.Append(parserResult.Instructions, parserFrame.Item2);
        }
